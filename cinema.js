...
let manifest=null,wanted='title',shown=null,active=-1,sequence=0,quiet=matchMedia('(prefers-reduced-motion: reduce)').matches,loading=null,failed=false,frames=0;
...
function metrics(v){Object.assign(root.dataset,{renderer:'prerendered-film',chapter:wanted,clip:v.dataset.clip||'',travel:v.loop?'settled':'moving',rendered:String(frames),worldTime:v.currentTime.toFixed(3),videoWidth:String(v.videoWidth),videoHeight:String(v.videoHeight),quiet:String(quiet)});}
...
async function start(path,loop,seq,destination){
 ...
 try{await load(v,path,seq);if(seq!==sequence)return;
  const old=active;active=next;v.classList.add('cinema-on');poster.hidden=true;
...
document.addEventListener('visibilitychange',()=>{if(document.hidden)slots.forEach(v=>v.pause());else if(active>=0)play(slots[active],sequence);});
