...
(()=>{
 ...
 const intervals=[],worldSamples=[];const drawStart=Number(document.getElementById("poolWorld")?.dataset.rendered||0);let start=null,previous=null,hidden=false,finished=false,nextScene=0;const initialVisible=document.visibilityState;const sizes={width:innerWidth,height:innerHeight,dpr:devicePixelRatio};
 ...
 const frame=t=>{if(finished)return;if(start===null){start=t;previous=t;}else{intervals.push(t-previous);previous=t;}if(canPreview&&t-start>=nextScene&&scene<indexes.length){worldSamples.push({...document.getElementById('poolWorld')?.dataset});select.value=String(Math.min(select.options.length-1,indexes[scene++]));select.dispatchEvent(new Event('change',{bubbles:true}));nextScene+=5000;}
