...
(()=>{
 ...
 panel.innerHTML='<b>本機流暢度量度</b><p>保持此頁在前景。預覽模式會自動切換畫面，正式遊戲不會推進。</p><button id="qualityRun">量度十秒</button><button id="qualitySave" disabled>下載結果</button><pre id="qualityResult" style="white-space:pre-wrap">尚未量度。</pre>';
 ...
 const intervals=[];let start=null,previous=null,hidden=false,finished=false,nextScene=0;const initialVisible=document.visibilityState;const sizes={width:innerWidth,height:innerHeight,dpr:devicePixelRatio};
 ...
 const frame=t=>{if(finished)return;if(start===null){start=t;previous=t;}else{intervals.push(t-previous);previous=t;}if(canPreview&&t-start>=nextScene&&scene<indexes.length){select.value=String(Math.min(select.options.length-1,indexes[scene++]));select.dispatchEvent(new Event('change',{bubbles:true}));nextScene+=1100;}
 if(t-start>=10000){finish();return;}requestAnimationFrame(frame);};requestAnimationFrame(frame);setTimeout(finish,15000);
 };
